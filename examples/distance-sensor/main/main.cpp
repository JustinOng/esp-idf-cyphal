...
IDFNode::Heap<IDFNode::DEFAULT_O1HEAP_SIZE>
    node_heap;
IDFNode node(node_heap.data(), node_heap.size(), esp_timer_get_time, sendCanardFrame);

KeyValueStorage kvstore;

static uint16_t node_id = 10;
static CanardPortID port_id_distance = std::numeric_limits<CanardPortID>::max();

const auto node_registry = node.create_registry();
const auto reg_ro_node_description = node_registry->route("uavcan.node.description", {true}, []() { return "distance-sensor"; });
const auto reg_rw_pub_distance_id = node_registry->expose("uavcan.pub.distance.id", {true}, port_id_distance);
const auto reg_ro_pub_distance_type = node_registry->route("uavcan.pub.distance.type", {true}, []() { return "uavcan.si.unit.length.Scalar"; });
...
void sensorTask(void *arg) {
  ...
  vl.i2cMasterInit(PIN_I2C_SDA, PIN_I2C_SCL);

  if (!vl.init()) {
    ESP_LOGE(TAG, "Failed to initialise VL53L0X");
    vTaskDelete(nullptr);
  }

  Publisher<uavcan::si::unit::length::Scalar_1_0> distance_pub;

  if (port_id_distance != std::numeric_limits<CanardPortID>::max()) {
    distance_pub = node.create_publisher<uavcan::si::unit::length::Scalar_1_0>(port_id_distance, 50 * 1000UL);
  }
  ...
  while (1) {
    ...
    if (res) {
      ESP_LOGI(TAG, "result = %" PRIu16, result_mm);

      if (distance_pub != nullptr) {
        uavcan::si::unit::length::Scalar_1_0 msg;
        msg.meter = (float)result_mm / 1000;
...
void app_main() {
  ...
  xTaskCreate(sensorTask, "sensor", 4096, nullptr, 10, nullptr);
